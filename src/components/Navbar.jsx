...
const Navbar = () => {
  ...
  return (
    <div onClick={() => { dispatch(setShowSuggestions(false)) }} className="sticky top-0 z-50 flex items-center justify-between p-2 h-14 sm:h-17 bg-white/80 backdrop-blur-md gap-2">
      <div className="flex h-14 sm:h-16 w-auto m-0.5 items-center overflow-hidden shrink-0">
        ...
        <Link to='/'><img className="w-20 sm:w-30 h-12 sm:h-17" src="/youtube.png" /></Link>
      </div>
      <div className="flex gap-2.5 flex-1 justify-center sm:justify-start max-w-2xl">
        <div onClick={(e) => e.stopPropagation()} className="relative flex items-center w-full sm:w-125 h-9 sm:h-10 border border-gray-300 rounded-full">
        ...
        <div className="hidden sm:flex bg-gray-100 w-10 h-10 text-2xl cursor-pointer items-center justify-center rounded-full hover:bg-gray-200"><FaMicrophone /></div>
      </div>
      ...
      <div className="hidden md:flex gap-3 shrink-0">
        ...
        <div className="w-10 h-10 text-2xl cursor-pointer flex justify-center items-center rounded-full hover:bg-gray-100"><GoBell /></div>
        <img className="dp w-10 h-10 rounded-full object-cover" src="/photo.png" />
      </div>

      {/* Mobile: sirf profile photo dikhega, Create/Bell chhupa diya */}
